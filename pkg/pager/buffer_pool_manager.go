...
func (b *bufferPoolManager) deletePage(pid pageID) error {
	...
	delete(b.table, pid)
	// Next, we will pin this page frame, so it cannot be returned as a victimizes
	// frame (because we are trying to delete it.)
	b.replacer.pin(fid)
	// Then we will instruct the storage manager to deallocate it the page, and
...
// with a nil error.
func (b *bufferPoolManager) getUsableFrame() (*frameID, error) {
	...
	if !foundInFreeList {
		...
		if &cf != nil {
			if cf.isDirty {
				...
				err := b.manager.write(cf.pid, cf.page)
				if err != nil {
