...
// diskManager is a storage storageManager for working with files
...
// getFileSize returns the current file sz
...
func (dm *diskManager) allocateSegment() error {
	...
	// get the current file sz
	size := dm.getFileSize()
	// truncate the file; grow by segment sz
...
// allocate checks the underlying sz of the file and grows it in
// segment sized (2MB) chunks at a time once it reaches an 80% full
// rate. After it checks the sz, it will potentially grow the file,
...
func (dm *diskManager) write(pid pageID, p page) error {
	...
	// Update the diskManager file sz if necessary.
	if offset >= dm.fsize {
...
// getFileSize is a helper function that returns the file sz for the
