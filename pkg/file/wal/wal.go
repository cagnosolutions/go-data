...
import (
	...
	"github.com/cagnosolutions/go-data/pkg/cache"
)
...
type Log struct {
	...
	scache     cache.LRU[int, *segment] // segment entries cache
}
...
func (l *Log) appendEntry(dst []byte, index uint64, data []byte) (
	out []byte,
	epos bpos,
) {
...
func appendJSONEntry(dst []byte, index uint64, data []byte) (
	out []byte,
	epos bpos,
) {
...
func (l *Log) clearCache() {
	l.scache.Range(
		func(k int, s *segment) bool {
			s.ebuf = nil
			s.epos = nil
			return true
		},
	)
	l.scache = cache.LRU[int, *segment]{}
	l.scache.Resize(l.opts.SegmentCacheSize)
}
