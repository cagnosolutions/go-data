...
func TestClockReplacer(t *testing.T) {
	...
	clockReplacer.Unpin(1)
	util.Equals(t, uint32(6), clockReplacer.Size())

	// Scenario: get three victims from the clock.
	var value *FrameID
	value = clockReplacer.Victim()
	util.Equals(t, FrameID(1), *value)
	value = clockReplacer.Victim()
	util.Equals(t, FrameID(2), *value)
	value = clockReplacer.Victim()
	util.Equals(t, FrameID(3), *value)

	// Scenario: pin elements in the replacer.
	// Note that 3 has already been victimized, so pinning 3 should have no effect.
	clockReplacer.Pin(3)
	clockReplacer.Pin(4)
	util.Equals(t, uint32(2), clockReplacer.Size())
	...
	value = clockReplacer.Victim()
	util.Equals(t, FrameID(5), *value)
	value = clockReplacer.Victim()
	util.Equals(t, FrameID(6), *value)
	value = clockReplacer.Victim()
	util.Equals(t, FrameID(4), *value)
}
