...
func newBucket[K comparable, V any](hashkey uint32, key K, value V) bucket[K, V] {
	return bucket[K, V]{
		dib:     1,
		hashkey: hashkey,
		entry: entry[K, V]{
...
func (b *bucket[K, V]) getHash() uint32 {
	return b.hashkey
}
...
func (b *bucket[K, V]) setHash(hash int) {
	b.hashkey = uint32(hash)
}

// checkHashAndKey checks if this bucket matches the specified hk and key
func (b *bucket[K, V]) checkHashAndKey(hashkey uint32, key K) bool {
...
func (b *bucket[K, V]) String() string {
	return fmt.Sprintf(
		...
		b.getDIB(), b.getHash(), b.key, b.val,
	)
}
