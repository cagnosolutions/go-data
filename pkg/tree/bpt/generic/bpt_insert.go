...
func (t *BPTree[K, V]) insert(k K, v V) bool {
	// if the root is nil, then the tree does not exist yet, start a new tree
	if t.root == nil {
		t.root = startNewTree[K, V](k, &record[K, V]{k, v})
		return false
	}
...
func startNewTree[K Key, V any](k K, ptr *record[K, V]) *node[K, V] {
...
func insertIntoLeaf[K Key, V any](leaf *node[K, V], k K, ptr *record[K, V]) /* *node */ {
	var i, insertionPoint int
	for insertionPoint < leaf.numKeys && leaf.keys[insertionPoint].Compare(k) < 0 {
		insertionPoint++
	}
...
func insertIntoLeafAfterSplitting[K Key, V any](root, leaf *node[K, V], k K, pointer *record[K, V]) *node[K, V] {

	// perform linear search to find index to insert new record
	var insertionIndex int
	for insertionIndex < order-1 && leaf.keys[insertionIndex].Compare(k) < 0 {
		insertionIndex++
	}

	// initialize temporary variables
	var i, j int
	var tempKeys [order]K
	...
	// create new leaf
	newLeaf := &node[K, V]{isLeaf: true} // makeLeaf()
...
func insertIntoParent[K Key, V any](root *node[K, V], left *node[K, V], k K, right *node[K, V]) *node[K, V] {
	...
	if left.parent.numKeys < order-1 {
		return insertIntoNode[K, V](root, left.parent, leftIndex, k, right)
	}

	// otherwise, it doesn't fit, so we need to split upward
	return insertIntoNodeAfterSplitting[K, V](root, left.parent, leftIndex, k, right)
}

// insertIntoNewRoot creates a new root for two subtrees and inserts the appropriate Key into the new root
func insertIntoNewRoot[K Key, V any](left *node[K, V], k K, right *node[K, V]) *node[K, V] {
	root := &node[K, V]{} // makeNode()
	root.keys[0] = k
...
func getLeftIndex[K Key, V any](parent, left *node[K, V]) int {
	var leftIndex int
	for leftIndex <= parent.numKeys && (*node[K, V])(parent.ptrs[leftIndex]) != left {
		leftIndex++
	}
	return leftIndex
}
...
func insertIntoNode[K Key, V any](root, n *node[K, V], leftIndex int, k K, right *node[K, V]) *node[K, V] {
...
func insertIntoNodeAfterSplitting[K Key, V any](
	root, oldNode *node[K, V], leftIndex int, k K, right *node[K, V],
) *node[K, V] {
	...
	var zeroKey K
	for i = 0; i < order; i++ {
		tempKeys[i] = zeroKey // zero values
	...
	return insertIntoParent[K, V](root, oldNode, kPrime, newNode)
}
