...
func (t *BPTree[K, V]) delete(k K) *record[K, V] {
	var old *record[K, V]
	keyLeaf, keyEntry := t.find(k)
	if keyEntry != nil && keyLeaf != nil {
		t.root = deleteEntry[K, V](t.root, keyLeaf, k, unsafe.Pointer(keyEntry))
		old = keyEntry
		keyEntry = nil
	}
...
func deleteEntry[K Key, V any](root, n *node[K, V], k K, pointer unsafe.Pointer) *node[K, V] {

	// initialize temporary variables
	var minKeys, kPrimeIndex, capacity int

	// remove the Key and value from the current node
	n = removeEntryFromNode[K, V](n, k, pointer)

	// if the node is the room node, make sure to adjust
	if n == root {
		return adjustRoot[K, V](root)
	}
	...
	neighborIndex := getNeighborIndex[K, V](n)
	if neighborIndex == -1 {
		kPrimeIndex = 0
	} else {
	...
	var neighbor *node[K, V]
	if neighborIndex == -1 {
		neighbor = (*node[K, V])(n.parent.ptrs[1])
	} else {
		neighbor = (*node[K, V])(n.parent.ptrs[neighborIndex])
	}
	...
	if neighbor.numKeys+n.numKeys < capacity {
		return coalesceNodes[K, V](root, n, neighbor, neighborIndex, kPrime)
	}

	// redistribute the nodes
	return redistributeNodes[K, V](root, n, neighbor, neighborIndex, kPrimeIndex, kPrime)
}

// removeEntryFromNode does just that
func removeEntryFromNode[K Key, V any](n *node[K, V], k K, pointer unsafe.Pointer) *node[K, V] {

	// remove the Key and shift the other keys accordingly
	var i, numPointers int
	for n.keys[i].Compare(k) != 0 {
		i++
	}
...
func adjustRoot[K Key, V any](root *node[K, V]) *node[K, V] {
...
// will return -1
func getNeighborIndex[K Key, V any](n *node[K, V]) int {
	var i int
	for i = 0; i <= n.parent.numKeys; i++ {
		if (*node[K, V])(n.parent.ptrs[i]) == n {
			return i - 1
		}
	}
...
func coalesceNodes[K Key, V any](root, n, neighbor *node[K, V], neighborIndex int, kPrime K) *node[K, V] {
	...
	root = deleteEntry[K, V](root, n.parent, kPrime, unsafe.Pointer(n))
	n = nil // free
	return root
}
...
func redistributeNodes[K Key, V any](
	root, n, neighbor *node[K, V], neighborIndex, kPrimeIndex int,
	kPrime K,
) *node[K, V] {
	...
	if neighborIndex != -1 {
		...
		if n.isLeaf {
			...
			tmp = (*node[K, V])(n.ptrs[n.numKeys+1])
			tmp.parent = n
